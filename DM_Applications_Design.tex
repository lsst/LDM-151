...
{\author{
    ...
    R.H. Lupton, J.F. Bosch, T. Axelrod, P. Price, \\
...
\begin{itemize}
    \item {\bf Level 1 Science Pipelines} are designed to generate Level 1 data products. These include:
    \begin{itemize}
    \item {\bf \em Image Differencing Pipeline} (\wbsDiffim), to create difference images, and detect and characterize sources in them.
    \item {\bf \em Association Pipeline} (\wbsAssocP), to associate detected sources with known objects.
...
Level 1 pipelines run as the data are being acquired. They primarily focus on image differencing, and the reduction of information extracted from difference images. The algorithms they employ are designed and chosen to complete processing on minute (alert production) to day (Solar System object pipelines) time scales. They are also rerun as a part of Data Release Production (DRP), potentially in somewhat different configurations to achieve greater precision at the expense of increased runtime.
    ...
    \item {\bf Level 2 Science Pipelines} run annually or semi-annualy (for the first year of data), and are designed to generate deep co-adds and catalogs stemming from analysis of direct image data.  These include:
    \begin{itemize}
        \item {\bf \em Single Frame Processing (``SFM'') Pipeline}, (\wbsSFM) to reduce acquired visits and detect and characterize astrophysical sources present in these visits. Though having its own WBS element, {\bf \em PSF Estimation}, (\wbsPSF) is functionally a part of the SFM pipeline.
    ...
    The calibration products pipeline is also rerun as a part of Data Release Processing. Global self-calibration steps run in DRP only.
       ...
       \item {\bf Science Data Quality Analysis (SDQA) pipelines and toolkits}, to enable collection, computation, visualization, monitoring and analysis of data quality metrics across all pipelines. These are divided into:
       \begin{itemize}
           \item {\bf \em SDQA Pipeline}, (\wbsSDQAP), that provides low-level data collection functionality for SDQA and
           \item {\bf \em SDQA Toolkit}, (\wbsSDQAT), that provides the visualization, analysis and monitoring capabilities for SDQA.
       \end{itemize}
...
\subsubsection{Level 1 Science Pipelines Overview}
...
%
\begin{enumerate}
\item A visit is acquired (\uc{Prepare for Observing}) and reduced (\uc{Process Raw Exposures to Calibrated Exposure}) to a single {\em visit image}. This includes cosmic ray rejection, instrumental signature removal\footnote{E.g., subtraction of bias and dark frames, flat fielding, bad pixel/column interpolation, etc.}, combining of snaps}, etc.
...
Within 24 hours of discovery, LSST DM system will perform {\em precovery} PSF forced photometry on any prior difference image overlapping the position of new \DIAObjects taken within the past 30 days (\uc{Perform Precovery Forced Photometry}).
\\
...
\subsubsection{Level 2 Science Pipelines Overview}
...
\begin{enumerate}
    ...
    \item {\em Relative calibration}: The survey is internally calibrated, both photometrically and astrometrically using the {\bf \em Astrometric} (\wbsAstroCal) and {\bf \em Photometric Calibration Pipelines} (\wbsPhotoCal). Relative zero point and astrometric corrections are computed for every visit.
...
%
\begin{itemize}
    \item Classes to represent and manipulate coordinates
    \item Classes to represent and manipulate images and exposures (images with associated metadata)
    \item Classess to represent and estimate backgrounds on images
    \item Classess to represent the geometry of the camera
...
\begin{itemize}
    ...
    \item The current design of classes that represent the PSF does not allow for color-dependent PSF terms. These will need to be added in construction.
...
A {\em spuriousness metric} using machine-learning techniques (Bloom et al. 2010) if needed will be developed to assist the discrimination of real sources and those caused by artifacts.
\\
...
The Association Pipeline has two key responsibilities: i) it shall be able to associate newly discovered \DIASources with previously known \DIAObjects and \SSObjects, and ii) it shall be able to associate \DIAObjects with known \Objects from the Level 2 catalogs.
...
For matches to \SSObjects, the \SSObject's ephemeris are to be computed by NightMOPS (functionally a part of the Moving Object Pipeline, \wbsMOPS). Matching to computed ephemeris is to be performed as if they were \DIAObjects.

When Level 1 data is reprocessed, a more accurate clustering algorithm (OPTICS; Ankerst et al. 1999) will be employed.
...
\section{Level 2 Science Pipelines}
...
\begin{itemize}
    ...
    \item Determination of zeropoint and aperture corrections
    \item PSF determination (per-CCD)
    \item WCS determination and astrometric registration of images
    \item Sky Background determination
...
\begin{itemize}
    ...
    \item A topl-level Task to run the SFM pipeline.
\end{itemize}
...
The absolute World Coordinate System (WCS) will be determined using an {\em astrometry.net} type algorithm (Lang et al. 2009), seeded with the approximate position of the boresite.

This module will also include the capability to perform relative registration of a set of images to enable coaddition and image differencing, using the {\em meas\_mosaic} registration algorithm of Furosawa et al. (2013) as the baseline.
...
A prototype implementation of all major components of SFM baseline design has been completed in LSST Final Design Phase. The achieved accuracy is at the level of state-of-the art codes today (e.g., SDSS, SExtractor). We expect it will be possible to transfer a significant fraction of the existing code into Construction, for continued improvement to meet LSST accuracy requirements.
\\

WCS determination and image registration modules are an exception, and will require extensive redesign and rewrite. Sky determination module will have to be enhanced to support multi-CCD fitting capability.
\\
...
In areas of significant stellar crowding (i.e., Galactic plane, star clusters), this approach lends itself to imposition of apropriate template priors (i.e., the correct template being that of a point source convolved with the PSF). This, effectively, makes the deblender into a crowded field code, allowing this baseline to satisfy the requirements for crowded field photometry.

\paragraph{Star/galaxy separation}
\label{alg:star-galaxy}

We baseline the star/galaxy separation algorithm based on the ratio of PSF and model fluxes. We use the extendedness criterion as defined by the HSC (Furosawa et al., 2013):
\begin{equation}
{\rm extendedness} = (0.95\times {\rm flux.gaussian} \lt {\rm flux.psf})\, ?\, 0.0 : 1.0 \nonumber
\end{equation}
...
\subsection{PSF Estimation (\wbsPSF)}

\subsubsection{Key Requirements}

PSF Estimation algorithms must enable the estimation of the point spread function with residual ellipticity correlations at the levels required by LSR-REQ-0097.
...
\begin{itemize}
    ...
    \item Estimate the atmospheric contribution by modelling the PSF from bright, isolated, stars, and interpolating using PCA decomposition or Kriging.
\end{itemize}
...
Gaia standard stars will be used to fix the global astrometric system. It is likely that the existence of Gaia catalogs may make a separate Astrometric Calibration Pipeline obsolete.
...
No prototype implementation exists at this time.
...
In addition, SDQA Pipeline shall provide low-level data collection functionallity to support software development in Construction and Operations.
...
The core of the toolkit will be designed around IPython notebooks (\url{http://ipython.org/notebook.html}), web-based interactive computational environments that can combine code execution, text, mathematics, plots and rich media into a single document.

Standard data visualization {\em aspects} will be realized with predefined set of notebooks with tests/analyses executed on a given SDQA repository. These aspects will allow the browsing and drill-down of data collected in Data Release Production runs, or monitoring of live data (for Level 1).

Data analysts and users will be able to create new notebooks to examine particular anomalies discovered in the data. It will be possible to add these notebooks to the library of predefined aspects, to be executed or monitored in Level 1 and Level 2 productions.
