...
\newcommand{\rednote}[1]{\noindent{\bf{\color{red} [NOTE: #1]}}}
\newcommand{\dlplink}[1]{{\color{blue}\href{http://jira.lsstcorp.org/browse/#1}{#1}}} % extra curly braces
                                                                                      % needed to color only the link blue
...
\begin{itemize}
    ...
    (e.g., {\tt AssembleCcdTask}, or {\tt CalibrateTask}).
    \item A top level class to apply corrections to the input data in the proper order. This top level class
    (and any sub-task therein) can be overridden in the instrument specific {\tt obs\_*} packages, making the core SFM pipeline camera agnostic.
\end{itemize}
...
\paragraph{Instrumental Signature Removal:} \rednote{\dlplink{DLP-262}}
...
\paragraph{Sky Background Determination:} \rednote{\dlplink{DLP-263}}
...
\paragraph{WCS determination and image registration} \rednote{This is actually a different WBS(02C.03.08), though
\dlplink{DLP-264} is in this WBS}

Initial absolute WCS determination will be done by matching with an appropriate reference catalog using an
optimized algorithm like the one described in \cite{tabur07}.  The WCS used internally will comprise multiple
transforms taking into account knowledge of the distortions inherent in the optical system.
...
Assemble CCD; Determine Aperture Correction; Determine PSF; Remove Instrument Signature; Detect Sources;
Determine Photometric Zeropoint; Measure Single Visit Sources; Determine WCS; Sum Exposures, Combine Raw
Exposures, Remove Exposure Artifacts; Determine Sky Background Model; Calibrate Exposure; Process Raw
Exposures to Calibrated Exposure; Perform Single Visit Processing; 
...
\subsubsection{Baseline design}
The current baseline describes handling DCR by binning the sky. It is fairly apparent this will not work in
practice.  We should come up with another baseline design that is more likely to work in the context of our
assumed observation strategy.
\paragraph{Template generation}\rednote{This is in 02C.04, but will be informed by \dlplink{DLP-278}, \dlplink{DLP-277}, and \dlplink{DLP-281}}
This will require taking into account differential chromatic refraction.
\paragraph{SFM}\rednote{\dlplink{DLP-267}}
Reduce the images using the single frame processing pipeline

\paragraph{Construct Difference} \rednote{\dlplink{DLP-274}}
Using calibrated exposure, select the appropriate template and map the template to the exposure airmass and
paralactic angle. In cases of particularly good seeing, correlate
the science image with its own PSF to prepare it for matched filter detection (so called pre-convolution). Warp template to the science
frame coordinate system. Compute the matching kernel between the template PSF and the (possibly smoothed) science
image PSF. Subtract the two images.

\paragraph{Detect and Measure} \rednote{\dlplink{DLP-277}}
Detect and measure any sources on the difference image. This will include point source, dipole and moving
source (trailed) measurement.  
...
\paragraph{Spuriousness Engine} \rednote{\dlplink{DLP-285}}
...
\subsubsection{Baseline design} \rednote{\dlplink{DLP-268}; This also requires NightMOPS (02C.03.06)
\dlplink{DLP-293}}

The baseline design for \DIASources to \DIAObject association and \DIAObject to \Object association is to use
simple nearest-neighbor search (with a proper probabilitstic foundation \citep{budavari08}) while taking proper motions and positional errors ellipses into account.
...
The \DIASources and \SSObjects will be merged into a single table and matching will be carried out in memory
on the node doing the processing (rather than in the database).
...
Prototype implementation of the baseline design has been completed in LSST Final Design Phase. A version of
the nearest-neighbor matching has been implemented as a part of the Application Framework, 
while clustering using OPTICS resides in the database-related ingest modules.
\\

\rednote{I don't think this is true anymore} The prototype code is available at \url{https://github.com/lsst/ap}. The current prototype, while functional, will require a partial redesign in Construction to address scalability and performance.
...
\paragraph{Event packet construction} \rednote{\dlplink{DLP-271}}

\paragraph{Event packet distribution} \rednote{\dlplink{DLP-273}}

\paragraph{Simple filtering engine} \rednote{\dlplink{DLP-272}}
...
\paragraph{Tracklet generation and collapse} \rednote{\dlplink{DLP-294} and \dlplink{DLP-296}}

\paragraph{Link tracklets} \rednote{\dlplink{DLP-306}}

\paragraph{Orbit fitting} \rednote{\dlplink{DLP-297}}
This will include situations where the sources are trailed in a single exposure.  In those cases, we will use
the extra information to further constrain the orbits.(\dlplink{DLP-298})
