...
-  Makes all LSST data available through an interface that utilizes, to
   ...
   at Data Access Centers (“DAC”) and at external sites.

| The overall architecture of the DMS is discussed in more detail in the
Data Management System Design (
`DMSD <https://docushare.lsstcorp.org/docushare/dsweb/Get/LDM-148>`__)
document. The overall architecture of the DMS is shown in
Figure [fig:DMS].
...
-  **Level 1** products are intended to support timely detection and
   ...
   science data products, a number of related Level 1 “SDQA” [2]_ data
...
-  **Level 3** products will be generated by the users of LSST, using
   ...
   user-contributed Level 3 data product pipelines into the DMS
...
-  **Level 1 Pipelines**, grouped under the **Alert Production** element
   ...
   are also rerun as a part of Data Release Production (DRP),
...
-  **Level 2 Pipelines** run annually or semi-annualy (for the first
   ...
   -  ***PSF Estimation Pipeline***, (WBS 02C.04.03), to estimate the
      PSF properties and variation across the focal plane for each
      visit, to the degree of precision required by the
      `SRD <https://docushare.lsstcorp.org/docushare/dsweb/Get/LPM-17>`__.
      Note that the work of this pipeline goes beyond the typical
      single-CCD PSF estimation present in the SFM pipeline.
...
-  **Calibration Pipelines** process the collected calibration data and
   ...
   Release Processing. Global self-calibration steps run in DRP only.
...
directly fed by the output data stream from the Camera SDS [3]_ during
observing. This data stream contains both unprocessed (raw) camera
images, and images that have been corrected for crosstalk by the SDS on
...
PSF forced photometry on any prior difference image overlapping the
...
In addition to these, the Moving Object Pipeline (MOPS; WBS 02C.03.06;
...
#. *Multi-epoch object characterization*. A set of measurements
   ...
   simultaneously fits PSF-convolved models to the objects multiple
...
-  **Level 0 QA** includes the extensive and thorough testing of the DM
   ...
   performance of calibration pipelines and MOPS), and to test for
...
-  The current design of classes that represent the PSF does not allow
   for intensity-dependent PSF terms. These will need to be added in
   construction.
...
characterization is done beyond what’s required for zero-point, PSF,
...
the point-spread function (PSF, the light distribution for a point
source, a critical ingredient to understanding the data and measuring
accurate fluxes and shapes). Both the star selector and PSF determiner
...
stars, the variation of the PSF across the CCD will be determined. The
baselined “principal-components” PSF (pcaPsf) determiner performs a
...
produces a spatially-varying PSF model that captures the most important
changes in the PSF over the CCD.

These algorithms are intended to be sufficient to enable Level 1
processing. More advanced PSF determination algorithms will be developed
in the PSF Estimation Pipeline (WBS 02C.04.03).
...
Assemble CCD; Determine Aperture Correction; Determine PSF; Remove
Instrument Signature; Detect Sources; Determine Photometric Zeropoint;
Measure Single Visit Sources; Determine WCS; Sum Exposures, Combine Raw
...
detection, centroiding, aperture and PSF photometry, and adaptive shape
...
these packets to VO Event Brokers, using standard IVOA protocols (eg.,
VOEvent Transport Protocol; VTP. End-users will primarily use these
...
specify using an SQL-like declarative language, or short snippets of
(likely Python) code.
...
The design of these components are explained in detail in the MOPS
Design Document (
`MOPSD <https://docushare.lsstcorp.org/docushare/dsweb/Get/LDM-156>`__).
...
area of significant uncertainty and risk. An extensive report on MOPS
prototyping and performance is available as a part of the MOPS Design
Document (
`MOPSD <https://docushare.lsstcorp.org/docushare/dsweb/Get/LDM-156>`__).
...
PSF Estimation pipeline must enable the estimation of the point spread
...
| LSST’s PSF models will be implemented as a plugin. Simple PSF
...
Current state-of-the-art PSF estimation methods typically use some basis
functions (e.g., :math:`\delta`-functions, PCA) and a spatial model
(e.g., polynomials) to estimate the PSF. This is not expected to be
sufficient to reach LSST requirements. We’ve therefore adopted a
baseline algorithm as follows:

-  Decompose the PSF into a component due to the atmosphere and a
...
-  Estimate the atmospheric contribution by modelling the PSF from
...
per-CCD estimates of the PSF and cutouts of bright stars (PSF estimation
candidates) as inputs.
| The PSF of thick deep depletion devices such as LSST’s is known to be
intensity dependent. We will estimate the zero-intensity PSF by
iteratively forward-modeling the estimated zero-intensity PSF until
convergence is achieved. The same forward-modelling algorithm will allow
us to estimate the PSF in undersampled imaging, as is expected for
...
straightforward to incorporate into the PSF Estimation pipeline.

The remaining components of the PSF Estimation pipeline have not been
...
Image Coaddition Pipeline shall produce PSF-matched and non-PSF-matched
...
When non-PSF-matched coadds are produced, the effective PSF and its
...
with a PSF varying in a continuous fashion over the field, unless the
data is deliberately degraded by convolving the inputs to a common PSF
(known as “PSF matching"). In order to enable dealing with the
discontinuous PSF, the Coaddition Pipeline will construct a “CoaddPsf”
(§[alg:coaddPsf]) PSF model, which is a sum of the PSFs of the input
...
capable of producing “PSF-matched coadds” using well known PSF-matching
algorithms.
...
image, we also combine the PSF models of all the input exposures, using
an approach similar to that devised by Jee and Tyson (2011). This
combination is lazy, in that we simply store the PSF models, their
...
the coadd PSF model at that location by coadding the PSF models of all
...
PSF as spatially constant in the region of an individual object.
...
PSF-matched Coadd Exposures; Create Template Exposures;
...
the CoaddPsf PSF and searching for peaks above the preset threshold.
...
independently estimated PSF and compared with the pixel data until
...
exposure, convolve it by the appropriate PSF, and compare it to the data
...
do not properly account for the PSF). While it may be possible to
...
The apertures will be PSF-corrected and *homogenized*, convolved to a
canonical circular PSF. This is in order to derive a definition of
...
will tend to be circular (due to smearing by the PSF). In the
...
spaced, PSF-homogenized, elliptical apertures, convolved to standard
seeing.
...
coadds, then a set of PSF-convolved models will be fit to the deblended
the children. The initial templates with then be replaced by these
models, convolved with the appropriate PSF, and flux assigned as before.
...
convolved with the PSF). This, effectively, makes the deblender into a
...
the ratio of PSF and model fluxes. We use the extendedness criterion as
...
| SDQA Pipeline shall provide low-level data collection functionality
...
SDQA Pipeline implementation will monitor and harvest the outputs and
logs of execution of other science pipelines, computing user-defined
metrics.

The metrics will be defined by extending appropriate SDQA Pipeline base
classes, and configuring them in the SDQA Pipeline configuration file
and/or on the command line.

The outputs of SDQA Pipeline runs will be stored into a SDQA repository
...
| Prototype implementation of the SDQA Pipeline baseline design has been
...
| SDQA Toolkit shall.provides the visualization, analysis and monitoring
capabilities for science quality data analysis. Its inputs will be
provided by the SDQA Pipeline.
...
set of web pages with tests/analyses executed on a given SDQA
...
Analyze SDQA Metrics;Correlate SDQA metric with other data;Correlate
SDQA metrics;Display SDQA Metrics;

Prototype Implementation
~~~~~~~~~~~~~~~~~~~~~~~~

| Prototype implementation of the SDQA Toolkit has been implemented in
...
.. [3]
   Science Array Data Acquisition (DAQ) Subsystem
