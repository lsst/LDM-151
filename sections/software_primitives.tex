...
Other notes:
\begin{itemize}
\item Want to support constant mask and unvertainty, probably via single-pixel images with zero strides.
...
Includes components:
\begin{description}
\item[MaskedImage] Image, mask, unvertainty.
...
\item Source selectors -- We need classes that know how to apply cuts to a catalog (or potentially do detection) and return the resulting catalog.
\item Matrix inversion/decomposition -- this is related to linear least squares, but I'm not sure that covers this as we may want sparse matrix functionality
\item Data discovery -- Simply ask questions like: "What data are at this location in this repository"
\item Reference catalog representation
\item Basis set decomposition
\item Visualization -- Image display (with responsive stretch and scaling), scatter plot, etc.
\item orbit propagation
\item orbit prediction
\item orbit fitting
\item tree structures -- maybe just having a KD-tree we can use in C++ and Python will be enough
\end{itemize}