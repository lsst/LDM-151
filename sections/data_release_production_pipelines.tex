...
The final output data products of the ImChar/JointCal iteration are the Source table and most of the CalExp (calibrated exposure) images.  The latter includes Image, Mask, Variance, Background, PSF, WCS, and PhotoCalib components that we will track separately.  The Mask component will be further updated by the \hyperref[sec:drpDiffIm] pipeline after the ImChar/JointCal iteration is complete.
...
\item[PSFMatchedCoadd] A coadd data product (\hyperref[sec:spImagesExposure]{Exposure} or subclass thereof) with a constant, predefined PSF.
...
\end{description}
It is possible that our algorithms will allow some coadd data products to be merged: we may be able to simultaneously optimize both depth and seeing by building \hyperref[sec:acCoaddDecorrelation]{Coadd Decorrelation}, and the template coadds may be the same (or be a superset) of the PSF-matched coadds.  These possibilities will be discussed further in Sections~\ref{sec:drpCoaddTemplate} and \ref{sec:drpDecorrelateCoadds}.
...
In the baseline plan, we expect to simply use a very large target PSF for all MatchedWarp images, allowing us to include all images without attempting to deconvolve (some extremely poor images may be rejected from the entirety of Data Release Processing, of course, if it is judged they contain negligible information or are uncharacterizable).  If this adversely affects subsequent processing in \hyperref[sec:drpBackgroundMatchAndReject]{BackgroundMatchAndReject}, we may need to develop an iterative approach in which we apply WarpAndPsfMatch only to better-seeing visits first, using a smaller target PSF, run \hyperref[sec:drpBackgroundMatchAndReject]{BackgroundMatchAndReject} on these, and then re-match everything to a larger target PSF and repeat with a larger set of input visits.  Another option would be to only run \hyperref[sec:drpBackgroundMatchAndReject]{BackgroundMatchAndReject} on the best-seeing images, and to obtain matched backgrounds and improved masks for poor images only in \hyperref[sec:drpDiffIm]{DiffIm} (which would also require that \hyperref[sec:drpFinalImChar]{FinalImChar} and \hyperref[sec:drpFinalJointCal]{FinalJointCal} be deferred until after \hyperref[sec:drpDiffIm]{DiffIm} for these images).
...
The outputs of BackgroundMarchAndReject are a PSFMatchedCoadd (one for every \{tract, patch, filter\}) and updated Background and Mask components for the CalExp product.  Because it is not built with the final relative photometric and astrometric calibration, this cannot be the final PSFMatchedCoadd used for coadd-based measurements.  We expect this to be our final estimate of the Background, though Section~\ref{sec:drpWarpAndPsfMatch} describes a scenario in which at least some background estimation may be deferred until \hyperref[sec:drpDiffIm]{DiffIm}.  We do not expect these to be our final estimate of the CalExp Masks, but we do expect them to be sufficient to generate a clean final Source table in \hyperref[sec:drpFinalImChar]{FinalImChar} and the final astromeric and (relative) photometric calibration in \hyperref[sec:drpFinalJointCal]{FinalJointCal}.
