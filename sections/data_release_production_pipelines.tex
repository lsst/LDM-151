...
It is possible that our algorithms will allow some coadd data products to be merged, which will also have implications on the pipelines used to build them.  There are two major branch points:
\begin{description}
...
This pipeline correlates CalExp images from a single visit with their own PSFs and then resamples them onto a single patch-level image.  As with \hyperref[sec:drpWarpAndPsfMatch]{WarpAndPsfMatch}, these operations will likely be performed separately.  Correlation is a basic feature of the \hyperref[sec:spConvolutionKernels]{Convolution Kernels} software primitive, and we can again delegate to \hyperref[sec:acWarping]{Image Warping} for resampling.  Unlike \hyperref[sec:drpWarpAndPsfMatch]{WarpAndPsfMatch}, it is unlikely we would consider changing the order of operations; while it is mathematically possible, it is almost certainly more difficult to resample before autocorrelation.

Here it is somewhat more likely that we would require a single algorithmic to correlate and resample simultaneously, in order to improve uncertainty handling or computational performance.

WarpAndPsfCorrelate generates the CorrelatedWarp intermediate product, and has the same data flow and parallelization characteristics as \hyperref[sec:drpWarpAndPsfMatch]{WarpAndPsfMatch}.
