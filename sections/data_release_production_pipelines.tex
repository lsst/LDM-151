...
In DeepAssociate, we perform a sophisticated spatial match of all CoaddSources and DIASources, generating tables of DIAObjects, Object candidates, and a table of unassociated DIASources that will be used to construct SSObjects in \hyperref[sec:drpSolarSystemPipeline]{SSP}.
...
\subsubsection{SolarSystemPipeline}
\label{sec:drpSolarSystemPipeline}
...
The high-fidelity reprocessing element can be achieved by re-running the AP SSP pipeline workflow, with small adjustments, on reprocessed \DIASource and \DIAObject catalog. The processing will be seeded by the most recent MPC catalog, and the end result will be submitted to the MPC.

The LSSTORB product will similarly re-use the AP SSP pipelines, but be seeded with an empty catalog. As no submissions to the MPC will be made for this processing, it will require a "mock MPC" component which will simulate the creation of a new MPCORB catalog from candidate submissions. This component is likely algorithmically straight-forward (i.e., a default may be to accept all candidates as real). An additional component may be required to link short-arc orbits between observing seasons (i.e., rediscoveries of a previously discovered object); the need for this component will be assessed based on simulations and early commissioning data.
