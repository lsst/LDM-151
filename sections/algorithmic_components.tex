...
We can project the per-chip coordinates into a flat projection in the focalplane.
...
The mechanics of image subtraction will depend on the choice of algorithm.  The performance of several variations are being considered \citep{DMTN-021}.  The algorithms differ primarily in their treatment of correlated noise and whether PSFs are measured directly or a matching kernel is used.
...
an ``afterburner'' rescaling of the A\&L difference image \citep{DMTN-021}.
...
The algorithm defines an order $m$ (default 6) size $m-1$ acyclic connected tree as the pattern to match between catalogs.
Trees are constructed and matched for the brightest $n$ stars (default = 50) in descending order until a match is found.
...
WCS ($> 3$ arcsec), blind matching will be undertaken (e.g., by using the
