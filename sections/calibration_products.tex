...
\alg Scripting to perform the necessary sweeps of the laser light source, and characterization of its output as the pulse energy will likely need to be normalized to. \rednote{Not clear to Merlin a) exactly why these exist and b) what a ``low spatial resolution'' \emph{flat field image} actually means. Construction shouldn't be hard though.}
...
\alg Created from measurements in \secsymbol\ref{sec:calibInputs:CBP}. \rednote{\xxx Check that the method cited above is what you think it is and will therefore be done with the products referenced above. }
...
\subsubsection{Atmospheric Absorption}\label{sec:CPP:output:atmosphericAbsorption}
