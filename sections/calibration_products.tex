...
\XXX{To do: Put in a link for the CBP, and upon first mention say Collimated Beam Projector (CBP)}
...
\XXX{Change to say just rotating camera. We will have tools for more complex things if necessary anyway.} We will need to write CBP dithering scripts (mask-specific raster scanning routines, and then re-raster scanning at a different M1 position for the previous focal plane positions to differentiate crosstalk and optical ghosts), code to perform this differentiation, and then make the measurement of the coupling coefficients. This will be fiddly as the sensor types will be relevant. \hyperref{http://iopscience.iop.org/article/10.1088/1748-0221/10/05/C05010}{}{}{POC Crosstalk reference} \XXX{add this as a proper citation and insert into bib.}
...
\XXX{Act on: I was planning to use the photometry of the spots (which define the flats we need) and ignoring the CBP ghost data except as a cross-check. I don't believe that the scattering model will be good enough, especially the ghoulies.}
...
Further to these ``standard'' camera calibration ingredients, we will also require an illumination/ghost correction, which will either be derived from star field observations or using the final CBP prototype for direct measurement.
