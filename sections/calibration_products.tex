...
\alg Given LSST's 2s readout, we do not expect to need to remove cosmic rays explicitly; a robust stacking algorithm should be sufficient. A prototype construction algorithm currently exists in \texttt{pipe\_drivers}. The final version must be configurable to use scalar-, vector- or array-type overscan subtraction.\footnote{ If the readout noise in any channel is too low (relative to the
...
The gain values for all amplifiers in the camera, in \electron/ADU; identical to \secsymbol\ref{sec:CPP:inputs:gain}, unless updated during operations, though this is thought to be likely to be necessary.
...
\alg This will be measured using the CBP spot projections. However, these levels can also be measured by saturating many stars in long sky exposures and dithering, with custom processing used to calculate the PSF from the unsaturated stars. Subsequent code will be developed to detect where saturation is occurring and calculate the saturation levels.
% note to self (mfl) why do we need to PSF exactly if this is a level in electrons?


\subsubsection{Crosstalk}\label{sec:CPP:output:crosstalk}
The crosstalk matrix element for every pair of amplifiers in the camera; identical to \secsymbol\ref{sec:CPP:output:crosstalk}, unless updated during operations. The probability that this will need to be updated is high, as the validity of these values depends on them being measured with the camera in its final configuration. This is due to crosstalk levels being determined by the inter-CCD and inter-raft capacitive couplings, which, though supposedly small (especially in the case of the inter-raft coupling), depend on the exact physical locations of all the circuit boards and flex cables with respect to one another. It is therefore necessary to be able to remeasure this on the mountain using the CBP using the \hyperref[sec:CPP:inputs:CBP:crosstalk]{CBP crosstalk dataset}.
...
\alg Measurement of CTE is subtle, though several established methods exist for doing so. Using the \fefiftyfive method may not be possible due to the possible lack of a radioisotope source in the camera, but the \emph{extended pixel edge response} (EPER) method and flat-field correlation method would both be possible using the existing input data products. We expect to be able to reuse the measurement algorithms from the Camera Team once they have been ported to run within the DM framework.
%See \XXX{insert citation} for further reading on these methods.
...
The procurement of the CBP hardware includes that of the necessary low-level control drivers/software. T\&S TCS own the task of taking the vendor-provided low-level routines and turning these into real-world usable routines by constructing higher level functions for \eg homing, slew-to-position, mount a given maks \etc.  It will still, however, remain to write scripts for the CBP, and interfaces with the OCS, to allow making all of the desired measurements, especially as several, if not all of these, require doing so in concert with the 8.4m.
\alg As well as writing the necessary scripts to acquire the raw data products outlined in \secsymbol\ref{sec:CPP:inputs}, it will also be necessary to deliver a coordinate transformation package to allow the CBP to maintain fixed position on the focal plane whilst illuminating different portions of the pupil, and vice versa.
...
\begin{itemize}
	\item Gain values 
	\subitem Less accuracy is needed here than for the main camera; a PTC-based measurement using flats will likely be sufficient if the data is binned to correct for the \bfeffect, and will therefore reuse the PTC-based algorithm from the 8.4m. 
...
xxx need to add a reference here to \hyperref[sec:CPP:output:spectrophotometricStandards]{spectrophotometric standards}.
...
The Calibration Products Production sections aims to provide all the ingredients necessary to photometrically calibrate the entire LSST survey, visit-by-visit and band-to-band, and thus arrive at a single photometric zero-point for the  survey. % This section aims to outline that process...

The effective end-to-end instrumental throughput, as a function of wavelength, focal-plane and pupil position, and time, is known from the \hyperref[sec:CPP:output:monoPhotoFlat]{ghost-corrected monochromatic flat-fields} and the \hyperref[sec:CPP:output:filterTransmission]{filter transmission functions}.

The atmospheric transmission as a function of wavelength, at the time each observation is made, is known at some position in the field by taking the ratio of  \hyperref[sec:CPP:output:spectrophotometricStandards]{stellar spectrum} of a bright ($8^{\text{th}}-10^{\text{th}}$ magnitude) star, as measured by the \auxtelescope, to the BP/RP spectrum as measured above the atmosphere by Gaia. It should be noted that both these measured spectra are subject to enhancement; the stellar spectra from Gaia ($R \approx 80-100$) can be fitted to the standard stellar spectral types, allowing us to determine their true spectrum, and, as described in \secsymbol\ref{sec:CPP:aux:atmosphericAbsorption}, the absorption features from the atmosphere can be fitted to an atmospheric transmission code (\eg MODTRAN/LIBTRAN, with the choice TBD), allowing us to enhance the measurement of the spectral absorption features present. How necessary it will be to improve the spectral resolution from Gaia depends both on their performance and the delivered performance of the \auxtelescope, as we need to match its performance at a minimum.

With the system response function and atmospheric transmission measured, and a per-visit corrections made, each visit is assigned a photometric zero-point by either 

This allows us to make per-object corrections for the atmospheric transmission, leaving just the overall flux level unknown.



It should be noted that it is not yet known whether the atmospheric transmission will vary significantly across LSST's field of view, and that this will likely be measured for the first time by LSST itself. Should it turn out that the atmospheric transmission varies on spatial scales relevant to the survey, we propose to make further per-visit corrections by measuring the variation in flux as a function of color/spectral classification for all Gaia sources across the field of view. Furthermore, and conversely, measurement of this variation will allow the spatial structure of the atmospheric transmission to be constrained, providing a null-test to validate not performing this correction should it not be necessary.
